...
describe("mergeStats", () => {
  it("passes through when one side is null", () => {
    const s = { observations: 5, onTimePct: 80, avgDelaySec: 60, p90DelaySec: 200 };
    expect(mergeStats(null, s)).toEqual(s);
    expect(mergeStats(s, null)).toEqual(s);
  });

  it("weights by observations", () => {
    const a = { observations: 100, onTimePct: 90, avgDelaySec: 30, p90DelaySec: 100 };
    const b = { observations: 300, onTimePct: 70, avgDelaySec: 90, p90DelaySec: 300 };
